...
def fill_answer(puzzle, grid, bbox_origin, cfg, stop=None):
    """把解涂到屏幕: 只点击解中为黑的格子(数字格/白格不点).

    puzzle: recognizer.Puzzle(centers 为相对识别输入图的坐标);
    grid  : solver 的 a×b 布尔矩阵(True=黑);
    bbox_origin: 框选区域左上角(物理像素), 识别图 = 该区域的截图.
    返回点击格数; stop(StopFlag) 置位时抛出 StopRequested.
    """
    import pyautogui
    pyautogui.PAUSE = 0.0
