...
键鼠安全约定:
    - 点击一律使用坐标瞬移(不播放移动动画), 不调用任何平滑移动接口;
      Windows 走 ctypes SetCursorPos+SendInput 快速路径, 其他平台
      回退 pyautogui(瞬移语义相同);
    - 作答过程中随时可按 ESC 中断(StopFlag 热键, 键盘钩子独立于点击
      路径), 中断后不再点击;
    - 点击坐标越界即中止; Windows 快速路径没有 pyautogui 的左上角
      FAILSAFE, 由 ESC 热键与越界检查兜底.
作答语义(马赛克特有): 点击 = 切换涂黑/未涂. 解为黑的未涂格点一下涂黑,
棋盘上已有的黑格若解为白则点一下取消 — 与扫雷"插旗/取消插旗"同构.
"""
import random
import sys
import threading
...
def select_region():
    """半透明全屏覆盖层, 用户从左上拖到右下框选马赛克题目.

    返回物理像素坐标 ((x0,y0), (x1,y1)); 框选可稍大于题目,
...
def calibrate_shift(nb, expanded_origin, puzzle, fallback, log):
    ...
    必须加上 bbox 原点换算成屏幕绝对坐标(扫雷项目实测: 漏加会把框选
    原点整个当成位移, 框选不在屏幕原点时校准必然误报移动并打飞点击
    基准). 偏移 >3px 返回平移后的原点, 否则返回 fallback.
    """
